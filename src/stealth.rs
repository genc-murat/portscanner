...
impl StealthScanner {
    ...
    async fn perform_syn_scan(
        ...
        use libc::{AF_INET, IPPROTO_TCP, SOCK_RAW, sendto, sockaddr, sockaddr_in, socket};
        use std::mem;
        ...
        if let IpAddr::V4(ipv4) = self.target {
            ...
            return Err("IPv6 not supported in this implementation".into());
        }
        ...
        let response_result = timeout(
            ...
            self.listen_for_response(sock_fd, port),
        )
        .await;
    ...
    fn build_syn_packet(&self, dest_port: u16) -> Result<Vec<u8>, Box<dyn std::error::Error>> {
        let mut packet = Vec::with_capacity(40); // IP header (20) + TCP header (20)

        // IP Header
        packet.extend(&self.build_ip_header(20)?); // TCP header length = 20

        // TCP Header
        packet.extend(&self.build_tcp_header(dest_port)?);

        Ok(packet)
    }

    fn build_ip_header(&self, tcp_len: u16) -> Result<Vec<u8>, Box<dyn std::error::Error>> {
    ...
    fn build_tcp_header(&self, dest_port: u16) -> Result<Vec<u8>, Box<dyn std::error::Error>> {
        let mut header = vec![0u8; 20];
    ...
    async fn listen_for_response(
        ...
        loop {
            ...
            if let Some(state) = self.parse_response(&buffer[..received as usize], target_port)? {
                return Ok(state);
            }
        }
    }
