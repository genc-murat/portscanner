...
fn port_parser_benchmark(c: &mut Criterion) {
    ...
    let test_cases = vec![
        ...
        ("complex", "21,22,23,25,53,80,110,143,443,993,995,8080-8090,9000-9010"),
    ];
...
fn service_detection_benchmark(c: &mut Criterion) {
    use port_scanner::service_detection::{ServiceDetector, format_service_info, ServiceInfo};
    ...
    for (name, banner) in test_banners {
        group.bench_with_input(BenchmarkId::new("analyze_banner", name), &banner, |b, banner| {
...
fn os_fingerprinting_benchmark(c: &mut Criterion) {
    use port_scanner::os_fingerprinting::{OSDetector, OSFingerprint, format_os_info, NetworkCharacteristics};
    ...
    let test_characteristics = vec![
        ...
        ("windows", NetworkCharacteristics {
            ttl: Some(128),
    ...
    for (name, characteristics) in test_characteristics {
        group.bench_with_input(BenchmarkId::new("match_signatures", name), &characteristics, |b, characteristics| {
            ...
            |b, &concurrency| {
                b.iter(|| {
                    rt.block_on(async {
                        ...
                        for _ in 0..100 { // Reduced from 1000 for faster benchmarks
...
fn network_utilities_benchmark(c: &mut Criterion) {
    ...
    group.bench_function("banner_cleaning", |b| {
        let test_banner = "HTTP/1.1 200 OK\r\nServer: Apache/2.4.41 (Ubuntu)\r\nContent-Type: text/html\r\n\r\n";
        b.iter(|| {
