...
impl PortScanner {
    ...
    pub async fn run(&self) {
        ...
        for &port in &self.ports {
            ...
            let handle = tokio::spawn(async move {
                ...
                scan_port(target, port, timeout_duration, grab_banner).await
            });
    ...
    fn display_results(&self, mut results: Vec<ScanResult>) {
        ...
        println!("\n{}", "=".repeat(70));
        println!(
            "Scan completed! Total time: {:.2}s",
            results.iter().map(|r| r.response_time).max().unwrap_or(0) as f64 / 1000.0
        );
    }
}
...
async fn scan_port(
    ...
    let service = if is_open { detect_service(port) } else { None };
...
async fn grab_dns_banner(stream: &mut TcpStream, timeout_duration: Duration) -> Option<String> {
    use tokio::io::{AsyncReadExt, AsyncWriteExt};

    // Simple DNS query packet (A record for google.com)
    let dns_query = [
    ...
    if stream.write_all(&dns_query).await.is_err() {
        return None;
    }

    let mut buffer = [0; 512];
    match timeout(timeout_duration, stream.read(&mut buffer)).await {
        Ok(Ok(n)) if n > 2 => Some("DNS Server".to_string()),
        _ => None,
    }
}
